(() => {
  ...
  const openInvitation = (instant = false) => {
    if (invitationOpened || !opening || !openButton) return;
    ...
    // Keep audio play inside the user's tap for iOS Safari.
    startMusic();

    // Two frames guarantee that Safari has painted the closed envelope before animation starts.
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => opening.classList.add('is-opening'));
    });

    // Reliable fallback even if animationend is skipped by a mobile browser.
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    openingTimer = window.setTimeout(finishOpening, reduced ? 1750 : 3180);
  };
  ...
  openButton.addEventListener('touchend', handleOpen, { passive: false });

  opening.querySelector('.envelope-stage')?.addEventListener('animationend', (event) => {
    if (event.animationName === 'envelopeLeave') finishOpening();
  });
